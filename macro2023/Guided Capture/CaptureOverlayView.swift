...
struct CaptureOverlayView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            HStack {
                if !shouldDisableCancelButton {
                    CancelButton(appModel: appModel)
            ...
            if shouldShowTutorial, let url = Bundle.main.url(
                ...
                withExtension: "mp4") {
                TutorialVideoView(appModel: appModel, url: url, isInReviewSheet: false)
